...
class _MaskOCRCamState extends State<MaskOCRCam> {
                ...
                .then(
                  (value) => 1 == 1
                      ? Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) =>
                                TestingPassportParameters(res: res),
                          ),
                        )
  ...
  List _ListDataValue() {
    RegExp expDate1 = new RegExp(
      '^(?:0[0-9]|[12][0-9]|3[01])/(?:0[0-9]|1[01|02])/\\d{4}\$',
    );
    RegExp expDate2 = new RegExp(
      '^(?:0[0-9]|[12][0-9]|3[01]) [A-Z]{3} \\d{4}\$',
    );
      ...
      return SizedBox(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            if ((_ListDataValue()[index]['format'] as RegExp).hasMatch(
              listResult[index],
            ))
            ...
            return Card(
              child: ListTile(
                title: Row(
                  ...
                  children: [
                    Expanded(
                      child: Text(_ListDataValue()[index]['title'].toString()),
                    ),
                    Text(
                      '${listResult[index]}',
                      style: TextStyle(
                ...
                trailing: _onRetakeCount >= 3
                    ? (_ListDataValue()[index]['format'] as RegExp).hasMatch(
                            listResult[index],
                          )
                          ? null
                          : IconButton(
                              onPressed: () => _onEditDocData(
                                ...
                                inputKeyboardType:
                                    _ListDataValue()[index]['inputType'],
                              ),
    ...
    if (_checkFormatValList.every((el) => el == true)) {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          icon: Icon(Icons.error_outline, color: Colors.red, size: 56),
          title: Text("Error!"),
          content: Text(
      ...
      if (widget.ocrSubType == 'greenIdCard') {
        ...
        if (listResult.length == 4) {
          ...
          if (tempList.contains('/')) {
            ...
            final tempRe = tempList.replaceAll(
              tempList.split('/').last,
